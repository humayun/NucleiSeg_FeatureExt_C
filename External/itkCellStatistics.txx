...
GenerateData()
{
  ...
  m_Relabel->SetAttribute( "PhysicalSize" );
  m_Relabel->Update();
 ...
 // convert the image into a collection of objects
    ...
    label++)
  {
    ...
    std::cout << shapeObject->GetElongation() << std::endl;
    std::cout << shapeObject->GetCentroid() << std::endl;
    std::cout << shapeObject->GetPrincipalMoments() << std::endl;
    std::cout << shapeObject->GetPrincipalAxes() << std::endl;
    std::cout << statObject->GetMean() << ' ';
    std::cout << statObject->GetSum() << ' ';

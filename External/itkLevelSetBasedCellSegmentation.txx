...
(ImageSpacingType spacing )
{
  ...
  {
    index[j] = 0;
    size[j] = ( 2 * static_cast<ImageSizeValueType>( 1.2 * m_LargestCellRadius/spacing[j] ) + 1 ) ;
    start[j] = - static_cast<ImageSizeValueType>( 1.2 * m_LargestCellRadius/spacing[j] );
  }
...
{
  typename ThresholdingFilterType::Pointer m_thresholder = ThresholdingFilterType::New();
...
(ImagePointer speed, int NumOfCells, ImageIndexType start, ImageSizeType size)
{
  ...
  CompImagePointer thresh = Threshold(geodesicActiveContour->GetOutput(), -1000, 0, NumOfCells+1, 0);
...
{
  typedef typename itk::ResampleImageFilter< CompImageType,CompImageType > ResampleFilter;
  typedef typename itk::NearestNeighborInterpolateImageFunction< CompImageType > Interpolator;

  // create the resample filter, transform and interpolator
  typename itk::AffineTransform<double,ImageDimension>::Pointer transform = itk::AffineTransform<double,ImageDimension>::New();
...
GenerateData()
{
  ...
  for( unsigned int j = 0; j < ImageDimension; j++ )
  {
    cellImageExtent[j] = static_cast<ImageIndexValueType>( 1.2 * m_LargestCellRadius/spacing[j] );
  }
  ...
  while( indexIt != seeds.end() )
  {
    ...
    if ( pOut == 0 )
    {
      ...
      for( unsigned int j=0;j<ImageDimension;j++ )
      {
        ...
        if ( static_cast< ImageSizeValueType > ( idx[j] + cellImageExtent[j] ) <= inputSize[j]-1 )
        {
      ...
      CompImagePointer subMask = GAC( m_ROIfilter->GetOutput(), NumOfCells, levelsetStart, levelsetSize);
...
PrintSelf( std::ostream& os, Indent indent ) const
{
  ...
  os << indent << "Largest cell radius:" << GetLargestCellRadius() << std::endl;
  os << indent << "seedValue:" << GetSeedValue() << std::endl;
  os << indent << "Max Iterations:" << GetIterations() << std::endl;
  os << indent << "propagationScaling:" << GetPropagationScaling() << std::endl;
